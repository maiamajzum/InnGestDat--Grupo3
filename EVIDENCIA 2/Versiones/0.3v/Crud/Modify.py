    ...
    if user:
        ...
        match uS:
            case "1":
                ...
                cursor.execute("UPDATE Evidencia2.usuario SET username = %s WHERE email = %s OR username = %s;", 
                               (nuevo_username, uSelection, uSelection))
                print("Username modificado exitosamente.")

            case "2":
                nuevo_password = input("Ingrese el nuevo Password: ")
                cursor.execute("UPDATE Evidencia2.usuario SET password = %s WHERE email = %s OR username = %s;", 
                               (nuevo_password, uSelection, uSelection))
                print("Password modificado exitosamente.")

            case "3":
                nuevo_email = input("Ingrese el nuevo Email: ")
                cursor.execute("UPDATE Evidencia2.usuario SET email = %s WHERE email = %s OR username = %s;", 
                               (nuevo_email, uSelection, uSelection))
